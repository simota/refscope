# Refscope Fleet Charter v2

> **Status:** COMMITTED
> **Mutability:** IMMUTABLE — supersede only via `docs/fleet-charter-v3.md`. Never edit this file in place except for the non-substantive correction exception defined in §6.1.
> **Date:** 2026-05-03
> **Authority:** User authority override of Magi D4 verdict (Magi D2 supersede pathway via §6 amendment process)
> **Source:** Supersedes `docs/fleet-charter.md` v1 (commit date: 2026-05-03)

---

## Diff from v1

| § | v1 wording (verbatim or summary) | v2 wording (verbatim or summary) | Reason | Authority |
|---|---|---|---|---|
| §1 Principle 2 | "Localhost only. No login, avatar, share-link, public URL, OAuth, JWT, multi-user state, server-side persistence." | "Localhost only. No login, avatar, share-link, public URL, OAuth, JWT, multi-user state. Server-side persistence is permitted ONLY for the user-managed repository allowlist (`~/.config/refscope/repos.json`); no other state may be persisted server-side." | Allow file-based persistence of UI-added repos; all other state remains memory-only | User authority override of Magi D4 |
| §1 Principle 3 | "The repository allowlist (RTGV_REPOS) is the only source of truth. Fleet view does NOT discover repos by scanning ~/, by reading .gitconfig, or by any auto-discovery mechanism." | "The repository allowlist source of truth is `RTGV_REPOS` env var **and the user-managed UI add list persisted at `~/.config/refscope/repos.json`**. Fleet view does NOT auto-discover repos by scanning ~/ or reading .gitconfig — all entries require explicit user action (env var or UI add button)." | Permit user-explicit UI add, retain anti-auto-discovery | Same |
| §3 Forbidden UI vocabulary | Implicit: included `"Add repository", "Open repository", "Browse for repository"` (per Magi D4 deferred decision) | Excluded from forbidden list. Team-feature vocabulary (`Team / Organization / Workspace / Members / Invite / Role / presence / Share / Login / Avatar`) remains forbidden. | Enable Add repository UI; preserve team-feature guardrail | Same |
| §6.2 6-month review baseline | (v1 commit date) | Reset to v2 commit date (2026-05-03) | New baseline for governance health review | Standard supersede practice |

---

## §1 Inviolable principles

The Fleet observation surface exists to satisfy a single user voice (Reo, on-call SRE, 12-20 repos). It is governed by 5 inviolable principles. A PR that crosses any of these principles cannot be merged as-is; **the path forward is to supersede this charter via `fleet-charter-v3.md`** (see §6 amendment) — no negotiation or case-by-case waiver is possible under this version.
...
2. **Localhost only.** The Fleet feature, like the rest of Refscope, runs on the user's single machine. No outbound network calls. No SaaS-shaped UI elements. Default `HOST=127.0.0.1`. Public bind requires explicit `RTGV_BIND_PUBLIC=1` opt-in with a printed warning. Server-side persistence is permitted ONLY for the user-managed repository allowlist (`~/.config/refscope/repos.json`); no other state may be persisted server-side.

3. **Single user.** Fleet is not a team feature. The repository allowlist source of truth is `RTGV_REPOS` env var **and the user-managed UI add list persisted at `~/.config/refscope/repos.json`**. Fleet view does NOT auto-discover repos by scanning ~/ or reading .gitconfig — all entries require explicit user action (env var or UI add button). There is no concept of "you", "owner", "team", "organization", "workspace", "member", "presence", "shared link", or "@mention". The user is the only consumer of their own Fleet surface.
...
There is no escape hatch. If a developer believes a forbidden token is justified, the only path is to supersede this charter (v3), which requires multiple reviewers and a documented rationale.
...
3. **Documentation (Layer 3):** This charter is immutable. Any change requires a new file (`docs/fleet-charter-v3.md`) that explicitly supersedes this one, signed by multiple CODEOWNERS reviewers.
...
### repos.json schema (version 1)

The user-managed repository allowlist persisted at `~/.config/refscope/repos.json` uses the following schema (version 1):
...
- `version`: integer, always `1` for this schema version.
- `repos`: array of repository entries added via the UI add button.
  - `id`: string, user-assigned or auto-derived repository identifier (must be unique, same validation rules as `RTGV_REPOS` IDs).
  - `path`: string, absolute path to the Git working tree root.
  - `addedAt`: string, ISO 8601 timestamp of when the entry was added via UI.
- Breaking changes to this schema (field removal, type narrowing) require a new `version` value and a migration path defined in a charter supersede.
...
The review output is a single ADR (`docs/adr/ADR-Fleet-Health-Review-2.md`) that recommends either (i) charter retention as-is, (ii) supersede to v3 with specific principle adjustments, or (iii) escalation to Magi for strategic re-deliberation.
...
The 6-month governance health review baseline is reset to the v2 commit date: **2026-05-03**. The next review is due by **2026-11-03**.
