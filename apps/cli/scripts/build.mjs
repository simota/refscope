...
//   2. Run `vite build` against the UI with VITE_RTGV_API_BASE_URL=""
//      so the UI fetches the API through relative paths.
//   3. Copy `apps/ui/dist/*` into `apps/cli/src/static/` so the same static
...
const UI_DIST = path.join(REPO_ROOT, "apps", "ui", "dist");
...
async function run() {
  ...
  log("building UI for relative API base");
  await buildUi();

  log("syncing UI dist into static/");
...
async function buildUi() {
  ...
  const uiDir = path.join(REPO_ROOT, "apps", "ui");
  const env = { ...process.env, VITE_RTGV_API_BASE_URL: "" };
  const localBin = path.join(uiDir, "node_modules", ".bin", "vite");
  ...
  if (viteAvailableLocally) {
    await runProcess(localBin, ["build"], { cwd: uiDir, env });
  } else {
    await runProcess("npx", ["--yes", "vite@6.4.2", "build"], { cwd: uiDir, env });
  }
}

async function syncStaticDist() {
  await resetDir(STATIC_DIR);
  await copyDir(UI_DIST, STATIC_DIR);
}
